    ...
    <div className="container mx-auto px-6 pt-32 pb-12 relative z-10">
      ...
      <div className="flex gap-4 mb-8">
        ...
        <button
          ...
          className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${activeTab === 'users' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'opacity-60 hover:opacity-100'}`}
        >
          Users
        </button>
      </div>
      ...
      {activeTab === 'overview' && stats?.growthData && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          {/* Company Growth Line Chart */}
          <GlassCard>
            <h3 className="text-xl font-display font-bold mb-6">Company Growth (Last 6 Months)</h3>
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={stats.growthData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  ...
                  />
                  <Legend />
                  <Line type="monotone" dataKey="users" stroke="#3b82f6" strokeWidth={3} name="Users" activeDot={{ r: 8 }} />
                  <Line type="monotone" dataKey="captains" stroke="#8b5cf6" strokeWidth={3} name="Captains" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </GlassCard>
          ...
          {/* Bookings Bar Chart */}
          <GlassCard>
            <h3 className="text-xl font-display font-bold mb-6">Booking Volume</h3>
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.growthData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#ffffff20" />
                  <XAxis dataKey="name" stroke="#ffffff80" />
                  <YAxis stroke="#ffffff80" />
                  <Tooltip
                  ...
                  />
                  <Legend />
                  <Bar dataKey="bookings" fill="#10b981" radius={[4, 4, 0, 0]} name="Completed Bookings" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </GlassCard>
              ...
              <table className="w-full text-left">
                <thead className="bg-white/5 text-xs uppercase tracking-wider opacity-70">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium text-right">Joined Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {users.map(u => (
                    <tr key={u._id} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-6 py-4 opacity-70">{u.email}</td>
                      <td className="px-6 py-4 opacity-70">{u.phone || 'N/A'}</td>
                  ...
                  {users.length === 0 && (
                    <tr>
                      <td colSpan="4" className="text-center py-12 opacity-50">No users registered yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
